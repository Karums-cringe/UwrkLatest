...
export default {
  data() {
    return {
      ...
      regNumber: '',
      location: '',
      userType: 'individual', // Default to individual type
      emailError: false,
      passwordError: false,
      nameError: false,
      surnameError: false,
  ...
  methods: {
    setRegisterChoice() {
      ...
      this.userType = 'individual' // Default to individual type
    },
    validateFields() {
    ...
    async register() {
      ...
      try {
        ...
        if (response.ok) {
          ...
          localStorage.setItem('userEmail', responseData.email) //user email in localhost
          this.$emit('login')
        } else {
